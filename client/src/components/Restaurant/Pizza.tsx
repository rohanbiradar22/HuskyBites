...
const PizzaCanvas: React.FC = () => {
  useEffect(() => {
    ...
    let map = (val: number, a1: number, a2: number, b1: number, b2: number) =>
      b1 + ((val - a1) * (b2 - b1)) / (a2 - a1);

    class Pizza {
      private canvas: HTMLCanvasElement;
      private ctx: CanvasRenderingContext2D;
      private sliceCount = 8;
      private sliceSize = 80;
      private width: number;
      private height: number;
      private center: number;
      private sliceDegree: number;
      private sliceRadians: number;
